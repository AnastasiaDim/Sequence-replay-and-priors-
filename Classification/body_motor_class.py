...
def analyze_eeg_data(participant_number):
    ...
    other_trials = df[(df['Image'].isin(['scrambled','scenes','tools'])) & (df['Block'].isin([1, 2, 3, 4]))].index.values
...
all_inter = []
# Loop through participants, skipping 10 and 41
for participant_number in range(1, 44):
    ...
    fname = f'P{participant_number}_weights_bodies_motor.xlsx'
...
export_accuracy.to_excel('accuracy_scores_body_motor.xlsx', index=False, header=False)

# Save all precision scores
export_precision = pd.DataFrame(all_precision_scores)
export_precision.to_excel('precision_scores_body_motor.xlsx', index=False, header=False)
...
std_accuracy_scores = np.std(all_accuracy_scores)

export4= pd.DataFrame(all_inter)
export4.to_excel('intercept_scores_bodies_motor.xlsx',index=False,header=False)  
# Save the average weights matrix
export_avg_weights = pd.DataFrame(average_weights_matrix)
export_avg_weights.to_excel('weights_scores_body_motor.xlsx', index=False, header=False)