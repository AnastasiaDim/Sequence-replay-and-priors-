...
def analyze_eeg_data(participant_number):
    ...
    category_trials = df[(df['Image'] == 'scrambled') & (df['Block'].isin([1, 2, 3, 4]))].index.values
    other_trials = df[(df['Image'].isin(['faces','scenes','tools','bodies'])) & (df['Block'].isin([1, 2, 3, 4]))].index.values
...
for participant_number in range(1, 44):
    ...
    fname = f'P{participant_number}_weights_scr_motor.xlsx'
...
export_accuracy.to_excel('accuracy_scores_scr_motor.xlsx', index=False, header=False)

# Save all precision scores
export_precision = pd.DataFrame(all_precision_scores)
export_precision.to_excel('precision_scores_scr_motor.xlsx', index=False, header=False)
...
export4.to_excel('intercept_scores_scr_motor.xlsx',index=False,header=False)  

# Save the average weights matrix
export_avg_weights = pd.DataFrame(average_weights_matrix)
export_avg_weights.to_excel('weights_scores_scr_motor.xlsx', index=False, header=False)