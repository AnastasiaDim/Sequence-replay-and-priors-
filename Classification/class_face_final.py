...
def analyze_eeg_data(participant_number):
    ...
    other_trials = df[(df['Image'].isin(['tools','scenes','bodies','scrambled'])) & (df['Block'].isin([1, 2, 3, 4]))].index.values
...
for participant_number in range(1, 44):  # Adjust the range for your participants
    ...
    fname = f'P{i}_weights_faces.xlsx'
    export3.to_excel(fname,index=False,header=False)
    export = pd.DataFrame(all_accuracy_scores)
    # Save DataFrame to Excel
    export.to_excel('accuracy_scores_faces_final.xlsx', index=False, header=False)
    ...
    #export2.to_excel('weights_scores_face_average.xlsx', index=False, header=False)
    
    export4= pd.DataFrame(intercepts)
    export.to_excel('intercept_scores_faces.xlsx',index=False,header=False)   

    export5= pd.DataFrame(all_precision_scores)
    export.to_excel('precision_scores_face.xlsx',index=False,header=False) 